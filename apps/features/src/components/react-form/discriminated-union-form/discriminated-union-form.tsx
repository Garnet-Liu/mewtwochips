...
const BuyNowBaseSchema = BaseSchema.extend({
  ...
  pricing: PricingType,
  spotsAvailable: z.coerce.number<number>().positive({ error: "Must be greater than 0." }),
});

const WhiteElephantBaseSchema = BaseSchema.extend({
  sell: z.literal(SellType.enum.WHITE_ELEPHANT),
  pricing: PricingType,
  quantity: z.coerce.number<number>().positive({ error: "Must be greater than 0." }),
});

const BuyNowAuctionSchema = BuyNowBaseSchema.extend({
  pricing: z.literal(PricingType.enum.AUCTION),
  priceInCents: z.coerce.number<number>().positive({ error: "Must be greater than 0." }),
});

const BuyNowSetPriceSchema = BuyNowBaseSchema.extend({
  pricing: z.literal(PricingType.enum.SET_PRICE),
  minimumBidAmountInCents: z.coerce.number<number>().positive({ error: "Must be greater than 0." }),
});
...
const WhiteElephantSetPriceSchema = WhiteElephantBaseSchema.extend({
  pricing: z.literal(PricingType.enum.SET_PRICE),
  minimumBid: z.coerce.number<number>().positive({ error: "Must be greater than 0." }),
});
...
const FormSchema = z
  ...
  .check((ctx) => {
    if (
      ctx.value.sell === SellType.enum.BUY_NOW &&
      ctx.value.pricing === PricingType.enum.SET_PRICE
    ) {
      if (ctx.value.minimumBidAmountInCents > 100) {
        ctx.issues.push({
