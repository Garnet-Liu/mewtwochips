...
export async function GET(request: Request) {
  ...
  try {
    const pokemonListResponse = await fetch(`https://pokeapi.co/api/v2/pokemon?offset=${offset}&limit=${limit}`);
    ...
    const fetchSpeciesResponseGroup: Array<Promise<PokemonSpecies>> = pokemonData.map(async (response) => {
