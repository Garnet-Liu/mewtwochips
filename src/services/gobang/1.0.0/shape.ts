...
const countShape = (
  ...
  for (let i = 1; i <= 5; i++) {
    const [nx, ny] = [x + i * offsetX + 1, y + i * offsetY + 1];
    const currentRole = board?.[nx]?.[ny];
    if (currentRole === undefined || currentRole === opponent) {
      break;
    }
    if (currentRole === role) {
      selfCount++;
...
export const getShapeFast = (
  ...
  // 有一点点优化效果：跳过为空的节点
  if (
    board[x + offsetX + 1][y + offsetY + 1] === EPiece.EMPTY &&
    board[x - offsetX + 1][y - offsetY + 1] === EPiece.EMPTY &&
    board[x + 2 * offsetX + 1][y + 2 * offsetY + 1] === EPiece.EMPTY &&
    board[x - 2 * offsetX + 1][y - 2 * offsetY + 1] === EPiece.EMPTY
  ) {
...
export const isFive = (shape: number) => {
  return shape === shapes.FIVE || shape === shapes.BLOCK_FIVE;
};

export const isFour = (shape: number) => {
...
export const getAllShapesOfPoint = (
  ...
  const roles = role ? [role] : [EPiece.BLACK, EPiece.WHITE];
  const result: number[] = [];
