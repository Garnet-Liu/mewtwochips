...
const Spin = forwardRef<HTMLElement, Readonly<SpinProps>>(function SpinRef(props, ref) {
  ...
  function OutChildren(props, ref) {
    ...
    if (loading) {
      return (
        <div
          ref={ref}
          className={cn("absolute inset-0 flex items-center justify-center bg-black/50", className)}
        >
          <LoaderCircle className="animate-spin" size={size} />
        </div>
      );
